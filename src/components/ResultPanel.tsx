...
import { Copy, Check, ExternalLink, Sparkles, CheckCircle2 } from "lucide-react";
import { toast } from "sonner";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader } from "@/components/ui/card";
import type { Angle } from "@/mastra/lib/schemas";

/** The three finished angles, each with a one-click "Copier le post". */
...
function AngleCard({ angle, index }: { angle: Angle; index: number }) {
  ...
  const copy = async () => {
    try {
      await navigator.clipboard.writeText(formatPost(angle));
      setCopied(true);
      ...
      <Card className="flex h-full flex-col border-node-angle/30">
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <span className="rounded-full bg-node-angle/15 px-2 py-0.5 text-[11px] font-semibold text-node-angle">
              Angle {index + 1}
            </span>
